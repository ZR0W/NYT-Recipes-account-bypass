...
A [Manifest V3](https://developer.chrome.com/docs/extensions/mv3/) browser extension for **desktop Firefox and Chromium** (Chrome, Edge, …), with Firefox for Android as a best-effort target. On a recipe page from a supported site, it summarizes the recipe already embedded in that page's own HTML into a small floating card — title, rating, time, yield, an interactive ingredients checklist, and numbered steps — no sign-in required, because it never needed sign-in in the first place: it just reads the structured recipe data the page ships to every visitor.

**Currently supported sites:** NYT Cooking (`cooking.nytimes.com`), babi.sh (`www.babi.sh`). The parser is built against the schema.org `Recipe` spec itself rather than any one site's markup, so adding another site is usually just a `manifest.json` entry — see `AGENTS.md`'s "Adding a new site" section.
...
- Not affiliated with, endorsed by, or reviewed by any of the sites it reads recipes from.

## Install (development — Firefox)

1. Open **about:debugging#/runtime/this-firefox**.
2. Click **Load Temporary Add-on…** and select **`manifest.json`** in this repo.
3. Visit a recipe page on a supported site (`cooking.nytimes.com` or `www.babi.sh`) — the card appears near the top of the page.
...
- The 🔒/🔓 badge next to the title isn't a guess — where a site publishes an `isAccessibleForFree` flag as part of its structured recipe data (NYT does this; babi.sh doesn't), the extension just displays whatever value the site put there. No badge appears at all on sites that don't publish this field — the extension never infers or checks paywall state itself.
